...
- `POST /api/feedback` takes an optional `kind` (`feedback` | `support`, see `shared/feedback-contract.ts`); without a valid one the server files a submission that carries a diagnostics snapshot as `support` and everything else as `feedback`, so older clients need no update
