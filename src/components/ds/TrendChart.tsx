...
export interface TrendChartPoint {
  /** X tick text ("14:00", "Jun 02"); series keys are numeric fields. */
  label: string;
  [seriesKey: string]: string | number;
}

export interface TrendChartBarSeries {
  key: string;
  name: string;
  /** Use chart tokens: var(--chart-sessions). Never the accent. */
  color: string;
}

export interface TrendChartAreaSeries extends TrendChartBarSeries {
  strokeWidth?: number;
}

export interface TrendChartProps {
  /** Points in order. `label` is the x tick; series keys are numeric fields. */
 ...
 */
export function TrendChart({ data, height = 280, areas = [], bars = [], yTicks = 4, minTickGap = 56 }: TrendChartProps) {
  ...
  return (
    ...
    >
      <svg width={width} height={height} style={{ display: "block" }}>
        ...
        {/* areas + lines */}
        {areas.map((s, si) => {
          const pts = data.map((d, i): [number, number] => [x(i), y(Number(d[s.key]) || 0)]);
      ...
      {hover !== null && hovered ? (
        ...
        >
          <p className="chart-tip-label">{hovered.label}</p>
          {[...areas, ...bars].map((s) => (
            <div className="chart-tip-row" key={s.key}>
              ...
              <span className="chart-tip-val">{(Number(hovered[s.key]) || 0).toLocaleString()}</span>
            </div>
          ))}
        </div>
      ) : null}
