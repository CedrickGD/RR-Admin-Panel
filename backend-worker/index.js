...
async function storeTelemetryD1(db, event, ownerInstallId, authMode) {
  ...
  if (event.service === SESSION_END) {
    await trimOldEvents(db);
  }
}

function isFreshHeartbeat(existing, event) {
  if (!existing || toNumber(existing.is_active) !== 1) {
    return false;
  }
...
async function bumpCounters(db, keys, updatedAt) {
...
function normalizeDisplayVersion(appVersion) {
  if (!appVersion) {
    return null;
  }
...
function incrementFeature(featuresJson, service) {
  let features = {};
  if (featuresJson) {
    try {
...
function readMetricBool(metrics, keys) {
...
function isBackgroundError(metrics) {
