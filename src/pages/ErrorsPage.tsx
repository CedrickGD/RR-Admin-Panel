  ...
  return (
    <div className="glass-inset" style={{ padding: "10px 12px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        <AlertTriangle
          size={13}
          style={{ flexShrink: 0, color: event.kind === BACKGROUND_KIND ? "var(--warning)" : "var(--danger)" }}
        />
        <span className="mono" style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--text-1)" }}>
        ...
        <span
          className="mono"
          style={{ marginLeft: "auto", fontSize: "0.6875rem", color: "var(--text-3)", whiteSpace: "nowrap" }}
          title={event.receivedAt ? `Received ${formatDate(event.receivedAt)}` : undefined}
        >
          {formatDate(event.timestamp)} · {timeAgo(event.timestamp)}
        </span>
      </div>
      <p style={{ margin: "6px 0 0", fontSize: "0.78125rem", color: "var(--text-1)", lineHeight: 1.55, wordBreak: "break-word" }}>
        {event.message?.trim() || "(no message)"}
      </p>
      {event.appVersion || event.sessionId || extras.length > 0 ? (
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 8 }}>
          {event.appVersion ? <span className="error-metric-tag">version: {event.appVersion}</span> : null}
          {event.sessionId ? (
            <span className="error-metric-tag" title={event.sessionId}>session: {shortId(event.sessionId)}</span>
          ) : null}
...
export function ErrorsPage() {
    ...
    const filtered = !q
      ...
      : visibleGroups.filter((group) => {
          const hay = [
            ...
            group.appVersion ?? "",
            group.deviceModel ?? "",
            ...group.visibleEvents.flatMap((event) => [event.type ?? "", event.message ?? "", event.code ?? ""]),
          ]
            .join(" ")
            .toLowerCase();
    ...
    for (const user of visibleGroups) {
      for (const event of user.visibleEvents) {
        ...
        if (existing) {
          ...
          if (parseTimestamp(event.timestamp) > parseTimestamp(existing.latest.timestamp)) existing.latest = event;
        } else {
          map.set(key, {
            key,
  ...
  function toggleFailure(key: string) {
    setExpandedFailures((curr) => (curr.includes(key) ? curr.filter((k) => k !== key) : [...curr, key]));
  }
        ...
        right={
          <>
            ...
            <div className="seg-control">
              ...
              ] as Array<{ key: ViewKey; label: string }>).map((t) => (
                <button
                  key={t.key}
                  type="button"
            ...
            <IconButton
              ...
              title={loading ? "Syncing" : "Refresh errors"}
            />
          </>
        }
      />
        ...
        right={
          <>
            <SearchInput
              value={query}
              onChange={setQuery}
              placeholder={view === "users" ? "Search user, Discord, error…" : "Search failure, user…"}
        ...
        {view === "users" ? (
          rows === null || rows.length > 0 ? (
                ...
                <tbody key={rows === null ? "loading" : "loaded"} className={rows === null ? undefined : "dt-settle"}>
                  {rows === null ? (
                    ...
                    rows.map((user) => {
                      ...
                      const events = showAll ? user.visibleEvents : user.visibleEvents.slice(0, EVENTS_PREVIEW_COUNT);
                        ...
                        <Fragment key={user.identity}>
                          ...
                          >
                            <td style={{ whiteSpace: "nowrap" }}>
                              <span style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "0.8125rem", marginRight: 6 }}>
                                {displayName(user)}
                              </span>
                              {!isUnattributed(user) ? (
                                <>
                                ...
                                <span style={{ fontSize: "0.6875rem", color: "var(--text-3)" }} title="These events carried no install id or hwid">
                            ...
                            <td>
                              <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                                ...
                                {showBackground && user.backgroundCount > 0 ? (
                                  <span style={{ fontSize: "0.6875rem", color: "var(--text-3)" }} title="Background task errors included in the count">
                            ...
                            <td style={{ whiteSpace: "nowrap", maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis" }}>
                              {top ? (
                                <span className="mono" style={{ fontSize: "0.71875rem", color: "var(--text-2)" }} title={top.type}>
                                  {top.type}
                                  {top.more > 0 ? <span style={{ color: "var(--text-3)" }}> +{top.more}</span> : null}
                                </span>
                              ) : (
                            ...
                            <td>
                              <IconButton
                                ...
                                style={{ padding: 4 }}
                                onClick={(e) => { e.stopPropagation(); toggleUser(user.identity); }}
                              ...
                              <td colSpan={USER_COLUMN_COUNT} className="row-expand-panel row-expand-td">
                                <RowExpandClip open={isExpanded}>
                                  ...
                                  <p className="label-sm" style={{ marginBottom: 8 }}>
                                    ...
                                    {hiddenBeyondCap > 0 ? ` of ${formatNumber(user.visibleCount)}` : ""})
                                  </p>
                                  ...
                                  {user.visibleEvents.length > EVENTS_PREVIEW_COUNT ? (
                                    <div style={{ display: "flex", justifyContent: "center", marginTop: 10 }}>
                                      <Button size="sm" onClick={() => toggleShowAllEvents(user.identity)}>
                                        {showAll ? "Show fewer" : `Show all ${formatNumber(user.visibleEvents.length)} errors`}
                                      </Button>
                                    </div>
                                  ) : null}
                                  {hiddenBeyondCap > 0 ? (
                                    <p style={{ fontSize: "0.71875rem", color: "var(--text-3)", marginTop: 10 }}>
                                      Only the latest {formatNumber(user.visibleEvents.length)} occurrences ship to the dashboard —{" "}
            ...
            <EmptyState icon={<Search />} title="No users match">
              No affected user matches “{query}”. Clear the search to see everyone with errors in range.
            </EmptyState>
          ) : (
            <EmptyState allClear>
              No {showBackground ? "" : "real "}failures in {rangeTitle.toLowerCase()}. New errors surface here within seconds of ingest.
            </EmptyState>
          )
              ...
              : failures.map((failure) => {
                  ...
                  return (
                    <div key={failure.key} className={`error-group${isOpen ? " is-open" : ""}`}>
                      <button type="button" className="error-group-row" onClick={() => toggleFailure(failure.key)}>
                        ...
                        <div className="error-group-meta">
                          ...
                          <Badge tone="warning" title={`${failure.count} occurrence${failure.count !== 1 ? "s" : ""}`}>
                            {failure.count}×
                          </Badge>
                          <Badge tone="muted" title="Distinct users hit by this failure">
                            {failure.identities.size} user{failure.identities.size !== 1 ? "s" : ""}
                          </Badge>
                          <span className="error-group-time muted-text" title={formatDate(failure.latest.timestamp)}>
                        ...
                        <div className="error-group-detail">
                          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 10 }}>
                            ...
                            <span className="muted-text" style={{ fontSize: "0.75rem" }}>
                              {failure.count} occurrence{failure.count !== 1 ? "s" : ""} across {failure.identities.size} user
                          ...
                          <div className="error-occurrence-list">
                            {shown.map(({ event, user }) => (
                              <div key={event.id} className="error-occurrence-item">
                                ...
                                <span className="mono muted-text" style={{ fontSize: "0.6875rem" }}>{formatDate(event.timestamp)}</span>
                                <span className="muted-text" style={{ fontSize: "0.75rem" }}>{timeAgo(event.timestamp)}</span>
                                {event.appVersion ? <Tag>{event.appVersion}</Tag> : null}
                              </div>
                            ))}
                          </div>
                          {failure.occurrences.length > shown.length ? (
                            <p style={{ fontSize: "0.71875rem", color: "var(--text-3)", marginTop: 8 }}>
                              Showing the latest {FAILURE_OCCURRENCES_SHOWN} of {formatNumber(failure.occurrences.length)} occurrences.
                            </p>
                          ) : null}
                        </div>
                      ) : null}
