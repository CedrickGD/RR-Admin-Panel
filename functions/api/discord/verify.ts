...
export async function onRequestPost(context: HandlerContext): Promise<Response> {
  try {
    ...
    const body = await readJsonBody<{ discord_id?: string; discord_tag?: string; license_key?: string; manual?: boolean }>(context.request);
    ...
    if (body.manual === true) {
      ...
      return json({ ok: true, verified: true, manual: true, discord_id: discordId, license_key: "MANUAL" });
    }
